...
(function (window, document) {
	...
	// 比如工单列表的下拉过滤器，当重新选择后就会变回原来的英文，此时并不触发 DOMNodeInserted 事件
	...
	function ignore(node) {
		...
		if (node.nodeType === Node.ELEMENT_NODE && node.tagName === 'LI' && node.hasAttribute('role') &&
	...
	function transElement(component, el, field, isAttr) {
		...
		if (transText === false) { // 无翻译则退出
			log("翻译失败, component = " + component + ", el = " + el + ", field = " + field + ", isAttr = " +
				isAttr);
			return false;
		}
